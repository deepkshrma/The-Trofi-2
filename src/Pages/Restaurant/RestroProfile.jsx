...
function RestroProfile() {
  ...
  function getImageUrl(src) {
    ...
    if (typeof src === "object") {
      const candidate =
        src.image || src.url || src.path || src.src || src.amenity_icon || src.icon;
  ...
  function renderAmenityItem(amenity, idx) {
    ...
    if (typeof amenity === "object") {
      const name = amenity.amenity_name || amenity.name || amenity.title || JSON.stringify(amenity);
  ...
  const amenitiesRaw = restaurant.amenities ?? restaurant.amenities_list ?? restaurant.facilities ?? null;
  ...
  return (
    <div
      className={`w-[100%] pt-[1.5rem] pb-[1rem] ${isToggle ? "pl-[19.3rem]" : ""} duration-900 min-h-screen `}
    >
      {/* Banner Carousel */}
      <div className="relative w-full h-100">
        <Carousel autoPlay infiniteLoop showThumbs={false} showStatus={false} interval={4000}>
          {bannerImages.length > 0 ? (
            bannerImages.map((img, i) => {
              ...
              const imgPath = typeof img === "string" ? img : img.image || img.url || img.path || img.src || "";
        ...
        <div className="absolute bottom-4 left-6 flex items-center gap-4 z-[1]">
          ...
          <div className="text-white">
            <h1 className="text-2xl font-bold">{restaurant.restro_name || restaurant.name}</h1>
            <p className="text-sm">{restaurant.description || restaurant.shortDesc}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        <div className="bg-white p-5 rounded-xl shadow-md">
          <h2 className="text-lg font-bold text-gray-800 mb-3">About {restaurant.restro_name || restaurant.name}</h2>
          <p className="text-gray-600">{longDescription}</p>

          <div className="mt-4 grid sm:grid-cols-2 gap-4 text-gray-700">
            <p className="flex items-center gap-2">
              <MapPin className="w-5 h-5 text-[#F9832B]" /> {restaurant.address || restaurant.location || "N/A"}
            </p>
            <p className="flex items-center gap-2">
              <Phone className="w-5 h-5 text-[#F9832B]" />{" "}
              {(restaurant.country_code ? `${restaurant.country_code} ` : "") + (restaurant.phone || "N/A")}
            </p>
            <p className="flex items-center gap-2">
              <Utensils className="w-5 h-5 text-[#F9832B]" />{" "}
              {Array.isArray(restaurant.food_type) ? restaurant.food_type.join(", ") : restaurant.food_type || "N/A"}
            </p>
            <p className="flex items-center gap-2">
        ...
        {/* Top Dishes Section */}
        {Array.isArray(restaurant.top_dishes) && restaurant.top_dishes.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Top Dishes</h2>
            <div className="bg-white rounded-xl shadow-md p-5">
              <ul className="divide-y divide-gray-200">
                {restaurant.top_dishes.map((dish, i) => (
                  <li key={dish._id || dish.id || i} className="flex justify-between items-center py-3">
                    <span className="text-gray-700 font-medium">{dish.name || dish.dish_name || "Untitled"}</span>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {menuImagesRaw.map((menu, i) => {
                const menuImg = typeof menu === "string" ? menu : menu.image || menu.url || menu.path || menu.src || "";
                const src = getImageUrl(menuImg);
                return (
                  <div key={i} className="bg-white rounded-xl shadow-md hover:shadow-lg transition p-3">
                    <img src={src} alt={`Menu-${i}`} className="w-full h-40 object-cover rounded-lg" />
                    <div className="mt-3">
            ...
            {hasLocation ? (
              <MapContainer center={[lat, lng]} zoom={15} style={{ height: "100%", width: "100%" }}>
                ...
                <Marker position={[lat, lng]}>
                  <Popup>
                    {restaurant.restro_name || restaurant.name} - {restaurant.address}
                  </Popup>
                </Marker>
                <FixMapSize />
              </MapContainer>
