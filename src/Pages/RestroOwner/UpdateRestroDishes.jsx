  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        // Categories
        const catRes = await axios.get(`${BASE_URL}/restro/get-dish-category-dropdown`, { headers });
        setDishCategories(catRes.data?.data || []);

        // SubCategories
        const subCatRes = await axios.get(`${BASE_URL}/restro/get-dish-sub-category-dropdown`, { headers });
        setDishSubCategories(subCatRes.data?.data || []);

        // Cuisines
        const cusRes = await axios.get(`${BASE_URL}/restro/get-cusine-dropdown`, { headers });
    ...
    <div className="main main_page min-h-screen py-10 px-6 lg:px-20 duration-900">
      <BreadcrumbsNav
        customTrail={[
          ...
          {
            label: "Dishes List",
            path: restaurantId ? `/RestaurantDishes/${restaurantId}` : "#",
          },
          { label: "Update Dish", path: `/UpdateRestroDishes/:id` },
        ]}
      />
...
export default UpdateRestroDishes;
