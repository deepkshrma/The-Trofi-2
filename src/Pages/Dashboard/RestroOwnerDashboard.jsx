...
//       if (topDishesSort === "Top Rated") {
//         return parseFloat(b.avgRating) - parseFloat(a.avgRating);
//       } else if (topDishesSort === "Newest") {
...
export default function RestroOwnerDashboard() {
  ...
  const [loading, setLoading] = useState(false);
  const [graphData, setGraphData] = useState({ ratingSeries: [], reviewSeries: [] });
  const [reviewCounts, setReviewCounts] = useState({ restaurant: 0, dish: 0 });

  const [ratingFilter, setRatingFilter] = useState("Monthly");
  const [reviewFilter, setReviewFilter] = useState("Monthly");
  const [topDishesSort, setTopDishesSort] = useState("Top Rated");
  ...
  // ✅ Fetch review counts (Restaurant and Dish separately)
  useEffect(() => {
    const fetchReviewCounts = async () => {
      try {
        ...
        // Fetch Restaurant reviews count
        const resRes = await axios.get(`${BASE_URL}/restrowner/restrowner-ratings`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { type: "Restaurant", typeId: restaurantId },
        });

        // Fetch Dish reviews count
        const dishRes = await axios.get(`${BASE_URL}/restrowner/restrowner-ratings`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { type: "Dish", typeId: restaurantId },
        });

        setReviewCounts({
          restaurant: resRes.data?.count || 0,
          dish: dishRes.data?.count || 0,
        });
      } catch (err) {
    ...
    if (restaurantId) fetchReviewCounts();
  }, [restaurantId]);
    ...
    <div className="p-6 main main_page min-h-screen duration-800 ease-in-out">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-6 mt-4">
        ...
        <StatCard
          ...
          value={reviewCounts.restaurant ?? 0}
          Icon={FiFileText}
          brand={BRAND}
          onClick={() => navigate(`/RestaurantReviews?tab=Restaurant`)}
        />

        <StatCard
          title="Dish Reviews"
          value={reviewCounts.dish ?? 0}
          Icon={FiFileText}
          brand={BRAND}
          onClick={() => navigate(`/RestaurantReviews?tab=Dish`)}
        />
